...
class SkillRegistry:
    ...
    def _parse_to_pydantic(self, data: dict[str, Any], skill_name: str = "unknown") -> SkillContract:
        ...
        KNOWN_KEYS = {
            ...
            "fs_scope",  # Filesystem scope constraint (e.g., "artifacts")
        }
...
class ScopeGate:
    ...
    def _get_git_changed_files(self, repo_path: Path | None = None) -> list[str]:
        ...
        # FIX #49: Exclude directories that are not scope-controlled
        # These are intermediate outputs, inputs, or reference implementations
        EXCLUDED_PREFIXES = (
            "artifacts/",       # Intermediate outputs
            "input_sources/",   # Input data sources
            "avidflow-back/",   # Reference implementation (not agent output)
            "reports/",         # Generated reports
            "nodepacks/",       # Installed node packages
            ".venv/",           # Virtual environment
            "__pycache__/",     # Python cache
        )
        
        # Dedupe and filter excluded paths
        return list(set(
...
class AdvisorOutputValidator:
    ...
    def validate_code_output(
        ...
        Args:
            code: Either a string of code, or a dict mapping filename -> code content
            correlation_id: Correlation ID for logging
            file_path: Optional file path for single-file validation
        
        Checks:
        - Sync-celery compatibility (no async, no background tasks)
        - Basic syntax validity (Python)
        """
        # Handle dict of filename -> code content
        if isinstance(code, dict):
            all_violations = []
            for fname, code_content in code.items():
                result = self.validate_code_output(code_content, correlation_id, file_path=fname)
                if not result.passed:
                    all_violations.extend(result.details.get("violations", [result.message]))
            if all_violations:
                return GateResult(
                    False,
                    f"Advisor code output failed validation: {len(all_violations)} issue(s)",
                    {"violations": all_violations},
                )
            return GateResult(True, "Advisor code output validated (all files)")
        
        # Ensure code is a string at this point
        if not isinstance(code, str):
            return GateResult(
                False,
                f"Expected string or dict for code, got {type(code).__name__}",
                {"type_received": type(code).__name__},
            )
    ...
    def execute(
        ...
        try:
            ...
            if contract.autonomy_level in (AutonomyLevel.IMPLEMENT, AutonomyLevel.COMMIT):
                ...
                # EXCEPTION: repo-ground skill creates repo_facts.json, so exempt it from check
                if skill_name != "repo-ground":
                    ...
                    ctx.log("repo_grounding_gate", {"passed": True, "exempted": True, "reason": "repo-ground creates repo_facts.json"})
                
                # 5b. Scope gate - files must be in allowlist AND git diff must be in scope
                # Policy: require_git_diff_scope_check: true
                # EXCEPTIONS:
                # - repo-ground: only writes to artifacts (fs_scope=artifacts)
                # - node-scaffold: creates allowlist.json (can't require what it produces)
                # - code-convert: only writes to artifacts (generated node code in artifacts/<id>/)
                # - test-generate: writes to tests/ which is in allowlist, but check_git would fail
                #   during development because of unrelated changes
                # - code-validate: reads/validates, doesn't write outside artifacts
                # FIX #50: During development, scope gate should not check ALL git changes against
                #          node-specific allowlist. That's meant for production agent execution.
                exempt_from_scope = {"repo-ground", "node-scaffold", "code-convert", "test-generate", "code-validate"}
                if skill_name not in exempt_from_scope:
                    ...
                    ctx.log("scope_gate", {"passed": True, "exempted": True, "reason": f"{skill_name} writes to artifacts only"})
            ...
            if execution_mode in (SkillExecutionMode.HYBRID, SkillExecutionMode.ADVISOR_ONLY):
                ...
                if files_modified:
                    # Filter out files in artifacts directory - these are intermediate outputs
                    # Only validate files that would be written to the actual repo
                    artifacts_prefix = str(ctx.artifacts_dir)
                    repo_files = [f for f in files_modified if not f.startswith(artifacts_prefix)]
                    ...
                    if repo_files:
                        ...
                        ctx.log("advisor_patch_validation_skipped", {
                            "reason": "all_files_in_artifacts_dir",
...
def _register_skill_implementations(executor: SkillExecutor) -> None:
    ...
    Supported function naming conventions:
    - execute_{skill_name}(ctx) -> dict  (e.g., execute_source_ingest)
    - run(ctx) -> dict
    """
    ...
    # Auto-discover all skills with impl.py files
    skill_impls = {}
    if skills_dir.exists():
        for skill_dir in skills_dir.iterdir():
            if skill_dir.is_dir():
                ...
                if impl_path.exists():
                    skill_impls[skill_dir.name] = impl_path
            ...
            try:
                ...
                if spec and spec.loader:
                    ...
                    if hasattr(module, func_name):
                        ...
                        executor.register_implementation(skill_name, getattr(module, "run"))
            except Exception:
