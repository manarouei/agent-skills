...
Generated: 2026-01-06T09:59:01.260201
...
class GithubNode(BaseNode):
    ...
    def _file_edit(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        file_path = self.get_node_parameter('filePath', item_index).rstrip('/')
        commit_message = self.get_node_parameter('commitMessage', item_index)
        binary_data = self.get_node_parameter('binaryData', item_index, default=False)
        
        # Extract optional additional parameters
        additional_parameters = self.get_node_parameter('additionalParameters', item_index, default={})
        ...
        # Get the current file SHA (required for editing)
        query = {}
        if additional_parameters.get('branch'):
            query['ref'] = additional_parameters['branch']
        
        # Fetch current file SHA
        sha_endpoint = f"/repos/{owner}/{repository}/contents/{file_path}"
        sha_response = self._api_request('GET', sha_endpoint, query=query if query else None)
        
        if 'sha' not in sha_response:
            raise ValueError('Could not get the SHA of the file.')
        
        body['sha'] = sha_response['sha']
        
        # Handle binary vs text content
        if binary_data:
            # Binary data: expect it's already base64 encoded from n8n
            binary_property_name = self.get_node_parameter('binaryPropertyName', item_index)
            # For converted code, assume binary data is in item_data['binary'][binary_property_name]['data']
            binary_info = item_data.get('binary', {}).get(binary_property_name, {})
            body['content'] = binary_info.get('data', '')
        else:
            # Text content: encode to base64 if needed
            file_content = self.get_node_parameter('fileContent', item_index)
            # Check if already base64 encoded (simple heuristic: no newlines/special chars)
            try:
                ...
                body['content'] = base64.b64encode(file_content.encode('utf-8')).decode('utf-8')
        
        # Add optional author information
        if additional_parameters.get('author'):
            author_info = additional_parameters['author']
            body['author'] = {
                'name': author_info.get('name', ''),
                'email': author_info.get('email', '')
            }
        
        # Add optional committer information
        if additional_parameters.get('committer'):
            committer_info = additional_parameters['committer']
            body['committer'] = {
        ...
        # Add optional branch
        if additional_parameters.get('branch'):
            body['branch'] = additional_parameters['branch']
        
        # Make the PUT request to update the file
        endpoint = f"/repos/{owner}/{repository}/contents/{file_path}"
        response = self._api_request('PUT', endpoint, body=body)
    ...
    def _file_delete(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        response = self._api_request('DELETE', f'/repos/{owner}/{repository}/contents/{file_path}', body=None, query=None)
    ...
    def _file_get(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        query = {}
        if additional_parameters.get('reference'):
    ...
    def _issue_create(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        response = self._api_request('POST', f'/repos/{owner}/{repository}/issues', body=None, query=None)
    ...
    def _issue_createComment(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        response = self._api_request('POST', f'/repos/{owner}/{repository}/issues/{issue_number}/comments', body=None, query=None)
    ...
    def _issue_edit(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        response = self._api_request('PATCH', f'/repos/{owner}/{repository}/issues/{issue_number}', body=None, query=None)
    ...
    def _issue_lock(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        response = self._api_request('PUT', f'/repos/{owner}/{repository}/issues/{issue_number}/lock', body=None, query=query)
    ...
    def _release_create(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        response = self._api_request('POST', f'/repos/{owner}/{repository}/releases', body=None, query=None)
    ...
    def _release_getAll(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        if return_all:
            response = self._api_request_all_items('GET', f'/repos/{owner}/{repository}/releases', body=None, query=query)
        else:
    ...
    def _release_update(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        response = self._api_request('PATCH', f'/repos/{owner}/{repository}/releases/{release_id}', body=None, query=None)
    ...
    def _repository_getIssues(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        if return_all:
            response = self._api_request_all_items('GET', f'/repos/{owner}/{repository}/issues', body=None, query=query)
        else:
    ...
    def _repository_getPullRequests(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        if return_all:
            response = self._api_request_all_items('GET', f'/repos/{owner}/{repository}/pulls', body=None, query=query)
        else:
    ...
    def _review_getAll(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        if return_all:
            response = self._api_request_all_items('GET', f'/repos/{owner}/{repository}/pulls/{pull_request_number}/reviews', body=None, query=query)
        else:
    ...
    def _review_create(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        response = self._api_request('POST', f'/repos/{owner}/{repository}/pulls/{pull_request_number}/reviews', body=None, query=None)
    ...
    def _review_update(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        response = self._api_request('PUT', f'/repos/{owner}/{repository}/pulls/{pull_request_number}/reviews/{review_id}', body=None, query=None)
    ...
    def _user_getRepositories(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        if return_all:
            response = self._api_request_all_items('GET', f'/users/{owner}/repos', body=None, query=query)
        else:
    ...
    def _user_invite(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        response = self._api_request('POST', f'/orgs/{org}/invitations', body=None, query=None)
    ...
    def _organization_getRepositories(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        if return_all:
            response = self._api_request_all_items('GET', f'/orgs/{owner}/repos', body=None, query=query)
        else:
    ...
    def _workflow_disable(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        response = self._api_request('PUT', f'/repos/{owner}/{repository}/actions/workflows/{workflow_id}/disable', body=None, query=None)
    ...
    def _workflow_dispatch(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        inputs_str = self.get_node_parameter('inputs', item_index, default='{}')
        owner = self.get_node_parameter('owner', item_index)
        repository = self.get_node_parameter('repository', item_index)
        ...
        try:
            ...
            raise ValueError(f"Invalid JSON in inputs parameter: {inputs_str}")
    ...
    def _workflow_enable(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Make API request
        response = self._api_request('PUT', f'/repos/{owner}/{repository}/actions/workflows/{workflow_id}/enable', body=None, query=None)
