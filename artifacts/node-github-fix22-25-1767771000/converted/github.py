...
Correlation ID: node-github-fix22-25-1767771000
Generated: 2026-01-07T07:39:44.792843
...
class GithubNode(BaseNode):
    ...
    def _api_request(
        ...
        if auth_type == 'oAuth2':
            credentials = self.get_credentials(f"{node_name_lower}OAuth2Api")
        else:
            credentials = self.get_credentials(f"{node_name_lower}Api")
    ...
    def _review_create(self, item_index: int, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Build request body
        body = {'event': event_text, 'event': body_text}
